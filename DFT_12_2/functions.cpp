...
int removeFrequnce()
{
	...
	selectPolygon(magMat, maskMat, fine);
...
int mouseROI()
{
	...
	selectPolygon(srcMat, dstMat, fine);
...
int main()
{
	...
	cv::Mat src = imread("C:\\Users\\Lenovo\\Pictures\\数图\\爱因斯坦.png", 0);
	cv::Mat src2 = imread("C:\\Users\\Lenovo\\Pictures\\数图\\玛丽莲梦露.png", 0);
	int m = getOptimalDFTSize(src.rows); //2,3,5的倍数有更高效率的傅里叶变换
	int n = getOptimalDFTSize(src.cols);
	Mat padded, padded2;
	//把灰度图像放在左上角,在右边和下边扩展图像,扩展部分填充为0;
