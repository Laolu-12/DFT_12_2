...
int removeFrequnce()
{
	cv::Mat srcMat = imread("C:\\Users\\Lenovo\\Pictures\\数图\\玛丽莲梦露.png", 0);
	cv::Mat magMat;
	cv::Mat phMat;
	cv::Mat maskMat;
	double normVal;
...
{
	cv::Mat dst;
	//cv::Mat src = imread("C:\\Users\\Lenovo\\Pictures\\数图\\玛丽莲梦露.png", 0);
...
int main() {
	...
	cv::Mat pic1 = imread("C:\\Users\\Lenovo\\Pictures\\数图\\玛丽莲梦露.png", 0);
	cv::Mat magMat1;
	cv::Mat phMat1;
	cv::Mat maskMat1;
	cv::Mat proceMag1;
	...
	//输出可视化的mag，以及相位谱，以及归一化系数
	calcVisbalDft(pic1, magMat1, phMat1, normVal1);

	magMat1 = magMat1 / 255;

	//在幅值谱上，通过鼠标选择，需要去掉的频率
	selectPolygon(magMat1, maskMat1);

	
	magMat1 = magMat1.mul(maskMat1);

	proceMag1 = magMat1 * 255;
	imwrite("处理后频谱.jpg", proceMag1);

	//逆变换
	calcDft2Image(magMat1, phMat1, normVal1, dst1);
	//dst1.convertTo(dstMat1, CV_8UC1);
	dst1 = dst1 / 255;
	dst1.copyTo(dstMat1);

	imshow("Input Image", pic1);    // Show the result
	imshow("处理后频谱", magMat1);
	imshow("逆变换结果", dstMat1);
